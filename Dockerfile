...
RUN apt install -y vim curl wget sudo git python3 usbutils awscli

RUN apt install -y adb android-libadb android-libbase android-liblog android-libcutils

RUN apt update --fix-missing
RUN apt install -y python3.8-dev python3.8-venv python3-setuptools \
...
RUN python3.8 -m pip install wrapt wheel cython
